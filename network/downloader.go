...
func DownloadManifestSync(realName string, saveDir string) {
	...
	downloadOne(entry, saveDir, manifest.PlatformAndroid, assetHeader, nil, counter, 1)
...
func DownloadAssetsAsync(catalog *manifest.Catalog, downloadDir string, platform string, keepPath *bool) {
	...
	for _, entry := range catalog.Entries {
		...
		go downloadOne(&entry, saveToDir, platform, assetHeader, sem, counter, dlAmount)
	}
...
func downloadOne(
	...
	request := prepareRequest(entry, platform, header)
